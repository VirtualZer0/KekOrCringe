...
`App.vue` intercepts every route change to play a `PageWipe` iris-and-stamp transition: each non-Main route declares a `meta.emoji`. Two `PageWipe` instances run back-to-back — an outgoing `phase="enter"` (clip-path circle iris grows from screen center, orange `var(--c4)`, while the destination emoji pops in on a chunky cream tile) followed by an incoming `phase="exit"` (same orange iris contracts back to a point while the tile pops out). The iris is a single fixed color across both phases — no per-route color flips, because every page in the redesign shares the same golden bg so there's no FROM/TO color to bridge. `v-if` + a per-transition `key` on each instance forces a fresh mount each navigation so the CSS animations actually re-run. Navigation is delayed `ROUTE_ANIM_MS` (~340ms) so the cover lands before the page mounts. Navigations to `Main` are special-cased to bypass the wipe (instant `next()`). Pending timers are tracked so a fast re-navigation supersedes the previous animation's state changes instead of letting stale timeouts fire against the newer route.
